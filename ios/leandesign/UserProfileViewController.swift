...
class UserProfileViewController: UIViewController {
    ...
    func obserUserInfo() {
        ...
        ref.observeSingleEventOfType(.Value, withBlock: { (snapshot) in
            if let imageUrl = snapshot.value!["profilePic"] as? String, let name = snapshot.value!["name"] as? String, let email = snapshot.value!["email"] as? String, let phone = snapshot.value!["phone"] as? String
            {
